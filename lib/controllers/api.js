    ...
    if (result.status === 0)  {
      ...
      client.udfRegister('udf/updateTweetsMap.lua', function(err) {
        if ( err.code === 0 ) {
          ...
          var UDF = {module:'updateTweetsMap', funcname: 'updateMap', args: [tweet,ts]};
...
exports.startsWith = function(req, res) {
  ...
  // NOTE: In production environment, this should be done via aql
  client.udfRegister('udf/startsWith.lua', function(err) {
    if ( err.code === aerospike.status.AEROSPIKE_OK ) {
      // udf registered  
      // console.error('startsWith registeration complete!');
      var statement = {aggregationUDF: {module: 'startsWith', funcname: 'find', args: [searchString]}};
      ...
      stream.on('error', function(err)  {
        console.log('startsWith Error:\n',err);
        res.json({status : err, users: []});
      });
      stream.on('end', function()  {
        // console.log('startsWith: ', '!done!');
      });
    } else {
      // An error occurred
      console.error('startsWith registeration error:\n', err);
      res.json({status : err, users: []});
    }
  });
